...
($b > 10) echo 0: echo 1; #Анологично с кодом
...
#Циклы
for ($k = 0;$k < 100;$k++){ break; continue; for() { break(2); } }
...
* Функции
*/
function fun()					{ static $x; $x++; }#Статическая переменная - относительно первого x это другая переменная, но со следующим вызовом переменная сохраняет своё значение
function fun(string $a, int $b)	{ global $x; $x++; }#Переменная становится глобальной и можно изменять её как первую
$func = function fun($a, $b) use ($num)	{ return $num++; }	#Использоание внешних функций (Замыкание)
function fun(&$a)				{ }					#Позволяет изменять входящую переменную
function fun()					{ yield 1; yield 2; } #Генератор возвращает все возможные значения с каждым вызовом можно почитат с помощью foreach
function fun()					{ yield from func(); } #Генератор возвращает ещё один генератор( возвраты суммируются )
function fun(...$args) {} #Сколь угодное количество аргументов записывается в массив
function &fun()					{}
$a = &fun();
